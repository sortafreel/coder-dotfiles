...
# --- Background work ---
# None block workspace start: ~5 repo clones, a marketplace clone plus 3 plugin installs, the slim stack selection, a phrocs refresh, the agent skills install, the bubblewrap apt install, and the Codex CLI install + login.
...
echo "coder-dotfiles: install.sh done (repo clones -> ~/.coder-dotfiles-clone.log, claude plugins -> ~/.coder-dotfiles-plugins.log, slim stack -> ~/.coder-dotfiles-slim-stack.log, phrocs -> ~/.coder-dotfiles-phrocs.log, skills -> ~/.coder-dotfiles-skills.log, bubblewrap -> ~/.coder-dotfiles-bubblewrap.log, codex -> ~/.coder-dotfiles-codex.log)"
